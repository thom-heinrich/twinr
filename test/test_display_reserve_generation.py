...
class DisplayReserveCopyGeneratorTests(unittest.TestCase):
    def test_rewrites_candidates_with_llm_generated_copy(self) -> None:
        ...
        self.assertIn("kleine Karte auf einem Screen", request["instructions"])
        self.assertIn("sofort verstehen worum es geht", request["instructions"])
        self.assertIn("Lust haben, mit Twinr in Interaktion zu gehen", request["instructions"])
        self.assertIn("klaren Themenanker", request["instructions"])
        self.assertIn("wie Kundenservice", request["instructions"])
        self.assertIn("Die headline muss fuer sich stehend als klare Aussage funktionieren.", request["instructions"])
        self.assertIn("Die body-Zeile ist der Call to Action.", request["instructions"])
