...
class TwinrHardwareLoop:
            ...
            def tts_worker() -> None:
                while True:
                    ...
                    try:
                        ...
                        # AUDIT-FIX(#1): Use the shared audio lock for every playback path to avoid overlapping recorder/player access.
                        with self._audio_lock:
    ...
    def _speak_full_answer(self, text: str, *, turn_started: float) -> tuple[int, int | None]:
        ...
        def playback_worker() -> None:
            try:
                # AUDIT-FIX(#1): Route synchronous answer playback through the shared audio lock as well.
                with self._audio_lock:
                    self.player.play_wav_chunks(mark_first_chunk())
            except Exception as exc:
