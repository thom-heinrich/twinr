...
- `display_impulses.py` and `display_impulse_copy.py` reuses the same generic state for non-voice ambient display moments: reserve-card candidates must stay positive and generic, with an explanatory headline plus a separate CTA line instead of hardcoding specific places, topics, or benchmark phrases
