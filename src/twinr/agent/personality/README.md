...
- keep prompt-time snapshot reads read-only and current-head-only, so first live turns on the Pi never reopen legacy snapshot or pointer/blob probes on the critical path
- keep explicit prompt-time remote-read caps authoritative across nested current-head helpers, so supervisor/personality prompt loads cannot silently widen a 2-second hot-path budget back to cold bootstrap timeouts
...
- expose one explicit persisted-history probe seam so nightly/runtime operators can distinguish a truly readable zero-delta state from remote-blind or partially unreadable personality history
...
| [display_impulse_copy.py](./display_impulse_copy.py) | Deterministic fallback copy for ambient display impulses that keeps the large headline explanatory and the smaller second line as the CTA when the bounded reserve-lane LLM rewrite is unavailable |
| [display_impulses.py](./display_impulses.py) | Silent ambient display-impulse candidates derived generically from mindshare and positive-engagement state, including structured generation context, semantic `card_intent` blocks for reserve-card rewriting, and a generic candidate-family label for reserve-bus planning mix |
...
| [store.py](./store.py) | Remote current-head/item persistence seam for personality snapshot state, signals, and deltas; prompt-time loads stay read-only and current-head-only so live voice turns do not reopen legacy snapshot probes |
...
| [service.py](./service.py) | Prompting-facing integration service plus the public read seam for persisted engagement signals used by the ambient reserve bus; short-lived caches key on stable remote-state identity so capped-timeout clones do not replay the same remote reads every turn |
...
- the ambient display lane may also surface a fresh continuity thread as one calm `hint` card before durable user engagement has fully formed, because the reserve bus is a lower-pressure surface than a spoken turn; `cooling` and `avoid` topics must still stay out
...
- `display_impulses.py` now also carries one generic `candidate_family` plus semantic `card_intent` per reserve-card candidate so the right-hand bus can mix world/place/memory/social families without ad-hoc named-topic spacing rules and the LLM rewrite step can write from structured meaning instead of only phrase-like anchors
- ambient reserve-card copy should make the headline itself explain the topic and use the smaller second line as the CTA, while still carrying one clear topical anchor so the user immediately knows which thread Twinr wants to open
