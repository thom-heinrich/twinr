"""Normalize bounded runtime setting changes for the base Twinr agent.

This module exposes the supported simple-setting catalog, helpers for reading
the current normalized values from ``TwinrConfig``, and the safe `.env`
persistence path used by tool handlers. Import from here when you need to
describe, validate, or persist one of Twinr's operator-adjustable settings.
"""
...
from twinr.agent.base_agent.conversation.language import language_display_name
...
class SimpleSettingUpdate:
    """Describe one normalized simple-setting update.

    Attributes:
        setting: Canonical setting name.
        label: Human-readable label for confirmations and logs.
        summary: Short rendered description of the resulting value.
        env_updates: Environment variables that must be persisted together.
        changed: Whether the normalized result differs from the current config.
        data: Structured metadata for callers that need previous/next details.
    """
...
def supported_setting_names() -> tuple[str, ...]:
    """Return the canonical simple-setting names supported by Twinr."""
...
def supported_spoken_voices(language: str | None = None) -> tuple[str, ...]:
    """Return the supported spoken-voice names for the current catalog.

    Args:
        language: Reserved for future language-specific catalogs. The current
            catalog is language-agnostic.
    """
...
def spoken_voice_options_context(*, language: str | None = None) -> str:
    """Render the supported spoken voices as prompt-ready catalog text.
    ...
    Returns:
        A semicolon-delimited string with each voice name and selection hint.
    """
...
def spoken_voice_language_note(language: str | None) -> str:
    """Return the user-facing note about voice language coverage."""
...
def adjustable_settings_context(config: TwinrConfig) -> str:
    """Build the prompt contract for the current simple-setting state.

    Args:
        config: Runtime configuration whose bounded setting values should be
            described to the model.

    Returns:
        Prompt text that lists the current adjustable settings, their current
        values, and the supported spoken-voice catalog.
    """
...
def update_simple_setting(
    ...
    *,
    ...
    """Normalize one bounded simple-setting change request.

    Args:
        config: Runtime configuration that provides the current values.
        setting: Canonical simple-setting name to modify.
        action: Requested action such as ``set``, ``increase``, or
            ``decrease`` depending on the setting.
        value: Optional requested target value for ``set`` actions.

    Returns:
        A normalized ``SimpleSettingUpdate`` describing the resulting env
        writes and rendered summary.

    Raises:
        ValueError: If the setting name, action, or requested value is not
            supported by the bounded setting surface.
    """
...
def write_env_updates(path: str | Path, updates: dict[str, str]) -> None:
    """Persist validated environment variable updates to a `.env` file.

    The existing file is read and rewritten atomically so comments and
    unrelated keys are preserved.

    Args:
        path: Target `.env` file path.
        updates: Mapping of environment variable names to their new values.

    Raises:
        OSError: If the file cannot be read or atomically rewritten.
        ValueError: If the target path or the supplied env keys/values are
            unsafe.
    """
...
def memory_capacity_level(config: TwinrConfig) -> tuple[int, str, int, int]:
    """Return the nearest supported memory-capacity preset for ``config``."""
...
def current_spoken_voice(config: TwinrConfig) -> str:
    """Return the normalized current spoken voice with Twinr fallback logic."""
...
def current_speech_speed(config: TwinrConfig) -> float:
    """Return the current normalized shared speech speed."""
...
def current_numeric_setting(config: TwinrConfig, setting: str) -> float | int:
    """Return the current normalized numeric setting value.

    Args:
        config: Runtime configuration to inspect.
        setting: Canonical numeric simple-setting name.

    Returns:
        The current normalized value, rendered as ``int`` or ``float`` to
        match the setting spec.

    Raises:
        KeyError: If ``setting`` is not a supported numeric simple setting.
    """
...
def _update_memory_capacity(
    ...
    *,
    ...
    """Build the normalized update for the memory-capacity preset."""
...
def _update_spoken_voice(
    ...
    *,
    ...
    """Build the normalized update for the spoken-voice setting."""
...
def _update_speech_speed(
    ...
    *,
    ...
    """Build the normalized update for the shared speech-speed setting."""
...
def _resolve_spoken_voice_option(requested_voice: str) -> dict[str, str] | None:
    """Resolve free-form voice text to the closest supported voice option."""
...
def _normalize_lookup_text(value: str) -> str:
    """Normalize lookup text for accent-insensitive voice matching."""
...
def _update_numeric_setting(
    ...
    *,
    ...
    """Build the normalized update for a bounded numeric simple setting."""
...
def _current_numeric_setting_value(config: TwinrConfig, setting: str) -> float:
    """Return the clamped current numeric setting value as ``float``."""
...
def _parse_requested_float(
    ...
    *,
    ...
    """Parse and clamp a requested numeric setting value."""
...
def _parse_memory_capacity_level(value: float | int | str) -> int:
    """Parse a requested memory-capacity level as an integer preset index."""
...
def _coerce_finite_float(
    ...
    *,
    ...
    """Coerce arbitrary input to a finite float within optional bounds."""
...
def _coerce_config_int(
    ...
    *,
    ...
    """Coerce arbitrary config input to an integer or a safe default."""
...
def _normalized_supported_voice_name(value: object) -> str | None:
    """Return a canonical supported voice name or ``None``."""
...
def _meaningful_lookup_tokens(value: str) -> set[str]:
    """Return non-trivial lookup tokens for voice matching."""
...
def _voice_option_lookup_tokens(option: dict[str, str]) -> set[str]:
    """Return the normalized token set that describes one voice option."""
...
def _validated_env_file_path(path: str | Path) -> Path:
    """Validate and normalize the target `.env` file path."""
...
def _ensure_no_symlink_components(path: Path) -> None:
    """Reject path chains that traverse symlinked directories."""
...
def _validated_env_updates(updates: dict[str, str]) -> dict[str, str]:
    """Validate env-key/value pairs before serialization."""
...
def _contains_unsafe_env_control_chars(value: str) -> bool:
    """Return whether an env value contains forbidden control characters."""

    return any(char in value for char in ("\x00", "\n", "\r"))


def _read_existing_env_lines(file_path: Path) -> list[str]:
    """Read an existing `.env` file without following symlink swaps."""
...
def _atomic_write_text(file_path: Path, content: str, *, encoding: str) -> None:
    """Atomically replace ``file_path`` with ``content``."""
...
def _fsync_directory(directory: Path) -> None:
    """Best-effort fsync for the directory that owns an updated file."""
...
def _quote_env_value(value: str) -> str:
    """Quote an env value when plain serialization would be ambiguous."""
