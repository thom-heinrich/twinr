"""Validate and bind the canonical realtime tool registry.

The registry maps externally visible tool names to ``RealtimeToolExecutor``
method names. This module keeps that mapping centralized and validates it
before runtime startup so broken handler wiring fails fast instead of during a
live conversation.
"""
...
_REALTIME_TOOL_BINDINGS: tuple[tuple[str, str], ...] = (
    ...
    ("browser_automation", "handle_browser_automation"),
    ("connect_service_integration", "handle_connect_service_integration"),
    ...
    ("list_smart_home_entities", "handle_list_smart_home_entities"),
    ("read_smart_home_state", "handle_read_smart_home_state"),
    ("control_smart_home_entities", "handle_control_smart_home_entities"),
    ("read_smart_home_sensor_stream", "handle_read_smart_home_sensor_stream"),
    ...
    ("manage_voice_quiet_mode", "handle_manage_voice_quiet_mode"),
...
def _validate_realtime_tool_bindings() -> tuple[str, ...]:
    """Validate the static registry and return the canonical tool-name order.

    Raises:
        RealtimeToolBindingError: If a binding entry is malformed or a tool
            name is duplicated.
    """
...
def _handler_accepts_payload(handler: Callable[..., Any]) -> bool:
    """Return whether a handler can accept the loop's single payload argument."""
...
def bind_realtime_tool_handlers(handler_owner: object) -> dict[str, RealtimeToolHandler]:
    """Bind the canonical realtime tool handlers from an executor-like owner.

    Args:
        handler_owner: Object that exposes the ``handle_*`` methods referenced
            by the static registry.

    Returns:
        A dictionary mapping tool names to validated bound callables.

    Raises:
        RealtimeToolBindingError: If the owner is missing a handler, exposes a
            non-callable attribute, or declares an incompatible signature.
    """
