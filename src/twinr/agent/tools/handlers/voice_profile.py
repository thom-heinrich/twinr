"""Handle voice-profile tool calls for realtime Twinr sessions.

Exports synchronous handlers for enrollment, status reads, and reset flows
around the local voice-profile monitor.
"""
...
def handle_enroll_voice_profile(owner: Any, arguments: dict[str, object]) -> dict[str, object]:
    """Enroll or refresh the local voice profile from the current turn audio.

    Args:
        owner: Tool executor owner exposing the voice-profile monitor, runtime,
            and current-turn audio.
        arguments: Tool payload with optional confirmation fields.

    Returns:
        JSON-safe payload with ``status="enrolled"``, sample statistics, and
        optional warning or live-assessment fields.

    Raises:
        MissingLiveAudioError: If no current-turn audio is available for
            enrollment.
        SensitiveActionConfirmationRequired: If spoken confirmation is required
            before storing the voice profile.
    """
...
def handle_get_voice_profile_status(owner: Any, arguments: dict[str, object]) -> dict[str, object]:
    """Read the current local voice-profile status.

    Args:
        owner: Tool executor owner exposing the voice-profile monitor and live
            runtime voice assessment.
        arguments: Unused tool payload kept for a stable handler signature.

    Returns:
        JSON-safe payload with enrollment state, sample statistics, last update
        timestamp, and current live voice signal information.
    """
    del arguments
...
def handle_reset_voice_profile(owner: Any, arguments: dict[str, object]) -> dict[str, object]:
    """Delete the local voice profile and clear live voice assessment state.

    Args:
        owner: Tool executor owner exposing the voice-profile monitor and
            runtime assessment state.
        arguments: Tool payload with optional confirmation fields.

    Returns:
        JSON-safe payload with ``status="reset"``, enrollment state, sample
        count, and optional warnings if cleanup partially degraded.

    Raises:
        SensitiveActionConfirmationRequired: If spoken confirmation is required
            before deleting the saved voice profile.
    """
