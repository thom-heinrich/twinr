"""Call a bounded local HTTP STT service from the server-side voice path.

The voice orchestrator runs on a GPU-backed server and can use a colocated STT
service for faster wakeword candidate scans than the generic keyword bundle.
This module owns only the HTTP client contract and WAV upload shaping; wakeword
matching stays in ``twinr.proactive.wakeword.matching`` and session policy
stays in ``voice_session.py``.
"""
...
class LocalSttServiceError(RuntimeError):
    """Raise when the local STT service rejects or malforms one transcription."""


@dataclass(frozen=True, slots=True)
class LocalSttTranscript:
    """Return one normalized local STT response."""
...
class LocalSttBackendAdapter:
    """Provide an OpenAI-like ``transcribe`` method backed by a local STT HTTP API."""
    ...
    def __init__(
        ...
        *,
        ...
        if not normalized_base_url:
            raise ValueError("Local STT base_url must not be empty.")
    ...
    def transcribe_capture(self, capture: AmbientAudioCaptureWindow) -> LocalSttTranscript:
        ...
        while True:
            try:
                ...
                if exc.code == 429 and attempt < self.retry_attempts:
                    ...
                    continue
                raise LocalSttServiceError(
                    f"Local STT service returned HTTP {exc.code}: {detail[:240]}"
                ) from exc
            except urllib_error.URLError as exc:
                raise LocalSttServiceError(f"Local STT service unavailable: {exc.reason}") from exc
        payload_text = payload_bytes.decode("utf-8", errors="replace").strip()
        if status >= 400:
            raise LocalSttServiceError(f"Local STT service returned HTTP {status}: {payload_text[:240]}")
        try:
            payload = json.loads(payload_text or "{}")
        except json.JSONDecodeError as exc:
            raise LocalSttServiceError("Local STT service returned invalid JSON.") from exc
...
def _encode_multipart_form(
    *,
    ...
    boundary = f"twinr-local-stt-{uuid4().hex}"
    chunks: list[bytes] = []
