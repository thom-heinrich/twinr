...
- fail closed when graph-contact fixture writes return `needs_clarification` or otherwise do not yield a concrete person node, so unified/messy evals never silently score a corrupted graph seed
- rebase ambiguous synthetic contact-lookup expectations against the actual mixed-corpus graph before scoring, so messy-eval writer/fresh-reader quality is not polluted by isolated synthetic-only option counts after extra contacts were intentionally composed into the shared namespace
- rerun only still-failing synthetic and multimodal cases until the shared-namespace visibility deadline expires, so the writer/fresh-reader benchmark measures recall quality instead of penalizing already-passing cases while remote current-head/catalog visibility catches up
- persist bounded multimodal failure diagnostics for both selector-side ids and the later unified-plan keep/drop decision, so future red cases show whether durable evidence died during store selection or during unified focus pruning
