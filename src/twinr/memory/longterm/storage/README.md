...
- let structured-store bootstrap trust those light remote probes when they already prove a valid snapshot head or catalog manifest exists, so watchdog recovery does not hydrate every `objects/conflicts/archive` segment just to decide nothing needs seeding
