...
- let projection-complete structured-memory and midterm writers skip async `job_status(...)` document-id waits only for fine-grained item batches when the catalog metadata already carries enough bounded selection payload for correct follow-up reads; immutable segment/current-head control-plane docs must still keep jobs-endpoint exact ids when available so retries or repeated namespaces cannot bind attestation to stale same-URI history
- keep those same projection-complete structured-memory and midterm writers on the async path for fine-grained item documents and catalog segments, but let their tiny one-item current-head control-plane writes bypass the async queue and use the sync fast path when exact item-id waits are already deferred
