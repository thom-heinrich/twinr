...
- route fine-grained remote catalog/segment writes through async `records/bulk` plus bounded-parallel same-URI readback attestation, so live device writes stop timing out on ChonkyDB's synchronous bulk ingestion path without serially re-reading every accepted item one by one
