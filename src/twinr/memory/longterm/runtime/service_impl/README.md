...
| `context.py` | Normal, live, fast, and tool-facing provider-context assembly, including bounded first-rewrite waits for sync/prewarm builds, strict materialized live-provider-front consumption, compatibility prepared full-context fallback, transcript-first speculative prewarm entry points, asynchronous seeding of the materialized live-provider front from compatibility provider builds, remote-memory completion stamps for the live voice-turn latency breakdown, fail-closed suppression of raw graph blocks when no relevant graph payload survived, and single-pass tool-context retrieval without duplicate remote durable/conflict reads while keeping graph loading as a fallback only when structured tool recall is empty |
