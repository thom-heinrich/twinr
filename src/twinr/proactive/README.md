...
keeps the broad `twinr.proactive` package-root surface alongside the focused
child packages that own runtime orchestration, social-trigger scoring,
short-window event fusion, delivery governance, and voice-activation behavior.

## Responsibility

`proactive` owns:
- expose the `twinr.proactive` package-root import surface across proactive child packages
...
- keep optional voice activation backend/tooling dependencies from breaking the root import surface when those paths are not selected at runtime
...
- remote transcript-first activation matching details in [`twinr.orchestrator`](../orchestrator/README.md)
