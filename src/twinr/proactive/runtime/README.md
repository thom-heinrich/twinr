...
- Let only the authoritative `display_attention_refresh` lane command the physical attention servo when that lane is available, so slower `automation_observation` snapshots cannot release and re-engage the servo between valid display ticks
...
- Keep HDMI attention-follow available on voice-activation/runtime-monitor builds even when `proactive_enabled` remains off for camera-triggered proactive prompts, including the transcript-first voice-orchestrator streaming runtime; the heavier gesture lane must stay idle there until the cheap attention path has fresh local hand/intent evidence, so the single live voice path remains real-time safe without losing deliberate gestures completely
- Enforce that transcript-first voice mode gates the dedicated HDMI gesture-refresh lane behind fresh attention-derived hand/intent semantics, so monitor/coordinator call sites cannot accidentally let the heavier gesture path free-run idle and reintroduce voice-sender starvation
...
| `attention_targeting.py` | Bounded multimodal attention-target prioritization over camera anchor, speaker association, showing intent, and session focus memory; speaker and last-motion targets keep priority in multi-person scenes, and active continuous visual targets keep their proved confidence floor instead of being re-averaged below the downstream servo follow gate |
...
| `continuous_attention.py` | Short-lived visible-person track matching, stabilized single-person anchor alignment, stale-track bridging across brief detector misses, last-motion fallback, adaptive audio mirror calibration, and continuous target-center prediction for HDMI gaze follow; visible speaker tracks keep a bounded follow-confidence floor so active body-follow does not release on weak transient fusion inputs |
| `display_attention.py` | Conservative proactive producer and local refresh policy for HDMI gaze-follow face cues, mirroring camera-space anchors into user-facing gaze, deriving bounded up/down follow from live person height, and renewing or briefly holding cues before they expire or disappear on short camera dropouts; `hdmi_wayland` attention refresh stays fail-closed while the shared streaming voice PID is sensitive to the hidden camera/servo lane, and continuous fullscreen face publication there also remains separately fail-closed pending dedicated display-side proof |
...
| `gesture_ack_lane.py` | Internal low-latency HDMI acknowledgement policy used by `perception_orchestrator.py`; it consumes only the authoritative perception-stream activation token, keeps runtime gesture calibration as the authoritative per-symbol confidence gate, and keeps just one HDMI-specific repeat hold locally |
| `gesture_stream_guards.py` | Shared user-intent provenance guard for HDMI gesture acknowledgements and visual wakeups; it requires current hand evidence and blocks stale/slow rescue sources from surfacing as deliberate live gestures |
...
| `service_impl/builder.py` | Default production assembly path, with wrapper-supplied dependencies to preserve legacy monkeypatch points while still allowing only safe HDMI attention lanes under transcript-first voice, fail-closing `hdmi_wayland` attention refresh until the Wayland memory path is fixed, and fail-closing same-device XVF3800 host-control polling that can starve live voice transport |
