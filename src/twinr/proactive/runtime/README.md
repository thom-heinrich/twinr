...
| `gesture_ack_lane.py` | Dedicated low-latency gesture acknowledgement stabilizer for the explicit user-facing symbol set, with Pi-tuned per-symbol ack floors, a minimum visibility window for thumbs/peace, and a current-hand-evidence gate separate from general camera-surface and wakeup policy |
...
| `gesture_wakeup_lane.py` | Dedicated visual wakeup stabilizer for one configured fine-hand symbol such as `peace_sign`, including a deliberate minimum visibility gate and current-hand-evidence requirement before dispatch, separate from both emoji acknowledgement and workflow orchestration |
