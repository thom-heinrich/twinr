...
- When the voice orchestrator owns the same XVF3800 that proactive audio would inspect, the proactive monitor must also fail-close ReSpeaker host-control polling in that streaming PID; HDMI attention may stay camera-only, but USB control snapshots must not compete with live voice transport.
...
- `display_attention.py` and `service_attention_helpers.py` may run the hidden `hdmi_wayland` camera/servo fast lane again only when fresh isolated Pi evidence proves the current attention-only path stays bounded in threads/RSS; continuous fullscreen face publication there must still stay fail-closed until separate display-side proof clears that risk.
...
- On transcript-first voice runtimes, fail-close the dedicated HDMI gesture refresh centrally in the shared helper contract. Do not re-arm the heavy gesture lane from local recent-hand semantics while it shares the streaming PID with live voice transport.
