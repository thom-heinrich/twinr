"""Bounded WhatsApp pairing helpers shared by web and voice surfaces.

The actual linked-device worker and auth persistence stay inside the WhatsApp
channel package, but pairing initiation should be reusable from more than the
web wizard. This module owns the reusable probe and bounded pairing window
logic.
"""
...
class WhatsAppRuntimeProbe:
    """Summarize the local WhatsApp worker prerequisites for pairing flows."""
...
SnapshotObserver = Callable[[ChannelPairingSnapshot], None]


@dataclass(slots=True)
class WhatsAppPairingCoordinator:
    """Run one bounded WhatsApp pairing window behind any Twinr surface."""
    ...
    def load_snapshot(self) -> ChannelPairingSnapshot:
        ...
        if not snapshot.running or self.registry.is_running(_WHATSAPP_CHANNEL_ID):
            self._notify_snapshot_observer(snapshot)
            return snapshot
        ...
        return self._save_snapshot(healed)
    ...
    def _run_pairing(self, config: TwinrConfig) -> None:
        ...
        try:
            while True:
                ...
                if remaining <= 0.0:
                    self._save_snapshot(_timeout_snapshot(snapshot))
                    return
                ...
                if event is None:
                    continue
                if not isinstance(event, WhatsAppWorkerStatusEvent):
                    continue

                snapshot = self._save_snapshot(_snapshot_for_status_event(snapshot, event))
    ...
    def _save_snapshot(self, snapshot: ChannelPairingSnapshot) -> ChannelPairingSnapshot:
        stored = self.store.save(snapshot)
        self._notify_snapshot_observer(stored)
        return stored

    def _notify_snapshot_observer(self, snapshot: ChannelPairingSnapshot) -> None:
        observer = self.snapshot_observer
        if observer is None:
            return
        try:
            observer(snapshot)
        except Exception:
            _LOGGER.warning("WhatsApp pairing snapshot observer failed.", exc_info=True)
...
def probe_whatsapp_runtime(config: TwinrConfig, *, env_path: Path) -> WhatsAppRuntimeProbe:
    """Collect bounded local runtime facts for WhatsApp pairing flows."""
...
def _snapshot_for_status_event(
    ...
    if event.qr_available and running:
        ...
        qr_image_data_url = event.qr_image_data_url or snapshot.qr_image_data_url
        ...
        qr_image_data_url = snapshot.qr_image_data_url if qr_needed else None
