...
- The channel also writes a bounded redacted inbound audit trail under `state/channels/whatsapp/debug/inbound_events.jsonl` with hashes, raw upsert provenance, and delivery outcomes so operator debugging can prove whether one or multiple upstream worker events triggered a send.
