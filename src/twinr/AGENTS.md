...
import surface, the `python -m twinr` bootstrap, and the small shared helper
modules reused across multiple subsystems. Structural ownership and
entrypoints live in [component.yaml](./component.yaml).
...
If `__init__.py` or `__main__.py` changed, also run:

```bash
PYTHONPATH=src pytest test/test_main.py test/test_config.py test/test_realtime_runner.py test/test_streaming_runner.py -q
```
