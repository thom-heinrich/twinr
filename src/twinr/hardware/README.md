...
| [respeaker_duplex_playback.py](./respeaker_duplex_playback.py) | Temporary silent-playback guard for bounded XVF3800 capture helpers that need the hardware's duplex playback state outside the long-lived voice orchestrator; productive voice guards stay inside the supervisor-owned streaming-loop session so restarts can reap them cleanly |
