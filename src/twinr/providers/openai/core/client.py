...
This module centralizes credential normalization, project-header selection, and
...
def _default_client_factory(config: TwinrConfig) -> Any:
    """Build the default synchronous OpenAI client from ``TwinrConfig``.

    Normalizes credentials plus timeout and retry settings before constructing
    ``openai.OpenAI`` so backend callers inherit the same request policy.
    """
