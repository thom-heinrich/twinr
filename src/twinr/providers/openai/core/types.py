...
code paths plus the filesystem and MIME validation helpers used to construct
safe image inputs.
"""
...
def _ensure_image_content_type(content_type: str) -> str:
    """Validate and normalize an image MIME type."""
...
def _sniff_image_content_type(data: bytes, filename: str) -> str:
    """Infer a supported image MIME type from file bytes and fallback hints."""
...
def _resolve_input_path(path: str | Path, *, base_dir: str | Path | None = None) -> Path:
    """Resolve an input path while rejecting symlink leaf nodes and escapes."""
    ...
    def from_path(
        ...
        *,
        ...
        Args:
            ...
            label: Optional text label that accompanies the image.
        ...
        Raises:
            ...
            ValueError: If the file is unsafe, unsupported, empty, or too large.
...
# Canonical replayable conversation history type shared across OpenAI callers.
