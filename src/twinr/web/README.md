...
- keep Conversation Lab on the same explicit durable-memory read/write tool surface as live Twinr turns, so saved-memory recall probes exercise the real runtime contract instead of hidden prompt-only behavior
