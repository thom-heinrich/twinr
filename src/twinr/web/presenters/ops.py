"""Format operational data for Twinr's web dashboard templates.

The functions here sanitize mixed-shape diagnostic payloads, redact secrets,
and normalize ops records into compact template-ready rows.
"""
...
def _snapshot_iterable(items: Iterable[Any] | None) -> tuple[Any, ...]:
    """Snapshot an optional iterable into a deterministic tuple."""
...
def _coalesce_display(value: Any, default: object) -> Any:
    """Replace explicit empty display values with a stable fallback."""
...
def _safe_field(source: object, name: str, default: Any = None) -> Any:
    """Read one field from mapping-like or attribute-backed records."""
...
def _mapping_view(value: Any) -> Mapping[Any, Any] | None:
    """Return a plain mapping view for supported diagnostic payload types."""
...
def _is_sensitive_key(key: str) -> bool:
    """Return whether a field name should always be redacted."""
...
def _redact_string_secrets(text: str) -> str:
    """Redact obvious credential assignments from free-form strings."""
...
def _sanitize_for_display(value: Any, *, _seen: set[int] | None = None, depth: int = 0) -> Any:
    """Convert arbitrary diagnostic payloads into redacted, display-safe data.

    Args:
        value: Arbitrary value from logs or usage metadata.
        _seen: Object ids already visited while walking nested structures.
        depth: Current recursion depth.

    Returns:
        A JSON-serializable structure or string with secrets redacted and
        recursion bounded for low-power dashboard rendering.
    """
...
def _safe_pretty_json(value: Any) -> str:
    """Render a bounded pretty-printed representation of diagnostic payloads."""
...
def _has_pretty_payload(value: Any) -> bool:
    """Return whether a payload should be rendered in expanded form."""
...
def _safe_number(value: Any) -> float | None:
    """Parse one finite numeric value for compact health formatting."""
...
def _format_log_rows(entries: Iterable[Mapping[str, object]] | None) -> tuple[dict[str, object], ...]:
    """Normalize ops log entries into template-ready rows with redacted data."""
...
def _format_usage_rows(records: Iterable[object] | None) -> tuple[dict[str, object], ...]:
    """Normalize provider usage records into template-ready dashboard rows."""
...
def _health_card_detail(health) -> str:
    """Build the compact detail line for dashboard health cards."""
