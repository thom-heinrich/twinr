"""Safely read and write file-backed web settings for Twinr.

This module owns hardened helpers for parsing URL-encoded form bodies,
round-tripping ``.env`` files, and atomically updating plain-text config
artifacts used by the local web control surface.
"""
...
def parse_urlencoded_form(body: bytes) -> dict[str, str]:
    """Parse one URL-encoded request body into a last-value mapping.

    Oversized or malformed payloads return an empty mapping instead of raising.
    """
...
def read_env_values(path: Path) -> dict[str, str]:
    """Read env assignments from ``path`` into a string dictionary."""
...
def write_env_updates(path: Path, updates: dict[str, str]) -> None:
    """Atomically merge validated env updates into ``path``."""
...
def read_text_file(path: Path) -> str:
    """Read a UTF-8 regular file, returning an empty string when missing."""
...
def write_text_file(path: Path, content: str) -> None:
    """Atomically replace ``path`` with normalized UTF-8 text content."""
...
def mask_secret(value: str | None) -> str:
    """Render a secret value as a short presence-only label."""
...
class FileBackedSetting:
    """Describe one env-backed field rendered by the web UI.

    Attributes:
        key: Backing env or file key saved by the form.
        label: Operator-facing field label.
        value: Current normalized value shown in the form.
        help_text: Secondary guidance shown below the field.
        tooltip_text: Additional contextual help for the field label.
        input_type: Field widget type such as ``text`` or ``select``.
        options: Select options as ``(value, label)`` pairs.
        placeholder: Placeholder text shown when the field is empty.
        rows: Requested textarea height in text rows.
        wide: Whether the field spans the wider layout treatment.
        secret: Whether the UI should treat the field as sensitive.
    """
...
def _quote_env_value(value: str) -> str:
    """Return a reversible ``.env`` representation for ``value``."""
...
def _validate_env_updates(updates: dict[str, str]) -> dict[str, str]:
    """Validate and normalize all env updates before persistence."""
...
def _validate_env_key(key: str) -> str:
    """Validate one env key and return its normalized form."""
...
def _validate_env_value(value: str) -> str:
    """Validate one env value before it is written to disk."""
...
def _split_env_assignment(raw_line: str) -> tuple[str, str] | None:
    """Split one raw env file line into ``(key, raw_value)`` if possible."""
...
def _parse_env_value(raw_value: str) -> str:
    """Parse one raw env value while preserving significant whitespace."""
...
def _parse_double_quoted_env_value(value: str) -> str:
    """Decode the quoted value format emitted by ``_quote_env_value``."""
...
def _get_path_lock(path: Path) -> threading.RLock:
    """Return the shared in-process lock guarding ``path`` writes."""
...
def _locked_path(path: Path) -> Iterator[None]:
    """Serialize access to ``path`` with in-process and OS-level locking."""
...
def _read_utf8_text(path: Path) -> str:
    """Read UTF-8 text from a regular file, returning an empty string if absent."""
...
def _open_regular_file_for_read(path: Path) -> int:
    """Open ``path`` for read-only access after regular-file checks."""
...
def _atomic_write_text(path: Path, content: str) -> None:
    """Atomically replace ``path`` with fully flushed UTF-8 text."""
...
def _ensure_parent_directory(directory: Path) -> None:
    """Ensure ``directory`` exists and is a real directory path."""
...
def _fsync_directory(directory: Path) -> None:
    """Flush directory metadata best-effort after atomic replacement."""
